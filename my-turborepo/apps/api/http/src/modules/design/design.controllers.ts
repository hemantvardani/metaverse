...
export const createAvatarHandler:RequestHandler = async (req:Request,res:Response,next:NextFunction):Promise<any>=>{
    ...
    try {
        ...
        const { isSuccess, data }= await createAvatar({img:safeParsed.data.img, title:safeParsed.data.title});

        if(!isSuccess){
            throw {};
        }

        responsePayload={status:'success', message:"Avatar created successfully", data:{...data.createdAvatar}};
